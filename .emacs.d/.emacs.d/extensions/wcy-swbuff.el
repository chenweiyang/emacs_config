...
(defun wcy-display-buffer-list (buffer-list)
  ...
  (let (message-log-max) ;; disable *Message* log
    (message "%s" 
             (concat (propertize (buffer-name (car buffer-list)) 'face wcy-switch-buffer-active-buffer-face)
                     "\n" 
                     (propertize (mapconcat  'buffer-name (cdr buffer-list)  "|")
...
if ARG is negative, switch backword, otherwise forward."
  ...
  (let* ((cycle-function (if (> arg 0) 'wcy-cycly-list ;; cycle forward or backward
    ...
    (or (eq 1 exitflag) (execute-kbd-macro key))
    nil))
